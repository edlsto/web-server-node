...
const forecast = ({ latitude: lat, longitude: long }, cb) => {
  ...
  request({ url, json: true }, (error, { body }) => {
    ...
    if (error) {
      ...
      cb(
        undefined,
        `${body.current.weather_descriptions[0]}. It is currently ${body.current.temperature} degrees out. It feels like ${body.current.feelslike} degrees out. The wind is currently blowing at ${body.current.wind_speed} mph.`
      );
    }
  });
};
